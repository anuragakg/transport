...
fetchOccupationList = () => {
	var url = conf.getOccupationList.url;
    var method = conf.getOccupationList.method;
    var data = {status : 1};
    TRIFED.asyncAjaxHit(url, 'GET', data, function (response, cb) {
        if (response) {
            addressData = response.data;
            fillOccupationList(response.data);
        } else {
...
fillOccupationList = (data) => {
	...
	$.each(data, function(i, data){ 
		...
		if(statusMaster){
			if (data.status == 1) {
				html += `<a class="data-active" onclick="utils.toggleStatus('${data.id}', conf.toggleOccupationStatus, this)"> Active</a>`;
			} else {
				html += `<a class="data-inactive" onclick="utils.toggleStatus('${data.id}', conf.toggleOccupationStatus, this)"> Inactive</a>`;
			}
		}
      	html += "</td></tr>";
	});
	$('#occupation-table tbody').html(html);
}
...
addOccupation = () => {
	...
	$('#formID').on('submit', function(e) {
		...
		if ($('#title').val().trim().length != 0) {
			...
			var url = conf.addOccupation.url;
	    	var method = conf.addOccupation.method;

			TRIFED.asyncAjaxHit(url, method, data, function (response) {
		        if (response.status == 1) {
		        	$('#formID')[0].reset();
		            TRIFED.showMessage('success', 'Successfully Added');
		            setTimeout(function() {document.location = "occupation-master.php";}, 500);
		        } else {
...
editOccupation = () => {
	...
	$('.data-edit').on('click',function(e){
		...
		var url = conf.getOccupationData.url + id;
		var method = conf.getOccupationData.method;
		var data = {};
		TRIFED.asyncAjaxHit(url, method, data, function (response, cb) {
			if (response) {
				addressData = response.data;
				fillOccupationData(addressData);
			} else {
				TRIFED.showMessage('error', cb);
			}
		});
...
updateData = () =>{
	...
	$('#updateButton').on('click', function(e) {
		...
		if ($('#updateTitle').val().trim().length != 0) {
			...
			var url = conf.updateOccupationData.url + id;
	    	var method = conf.updateOccupationData.method;
